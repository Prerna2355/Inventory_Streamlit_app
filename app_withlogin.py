...
def mask_phone(phone):
    phone = str(phone)
    return phone[:2] + "XXXX" + phone[-2:] if len(phone) >= 4 else "XXXX"
...
def normalize_area(area: str) -> str:
    return (
        area.strip()          # remove spaces
            .title()          # Sector 14, Dlf Phase 1
            .replace("  ", " ")
    )
...
def fetch_inventory():
    ...
    df = pd.read_sql_query(
        "SELECT id, Your_name AS 'Your_Name' Category AS 'Category',Property_Type AS 'Property_Type', Project_Name AS 'Project_Name', Address AS 'Address', Area AS 'Area', phone_number AS 'phone_number', Owner_name AS 'Owner_Name',price AS 'price', Cheque As 'Cheque', Size AS 'Size',Comments AS 'Comments' FROM Lead_inventory1",
        conn
    )
    conn.close()
    return df

def insert_item(Your_name,Category,Property_Type, Project_Name,address, Area, phone_number, Owner_name, price,Cheque,Size, comments):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO Lead_inventory1 (Your_name,Category, Property_Type, Project_Name,address, Area, phone_number, Owner_name, price,Cheque, Size, Comments) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)",
        (Your_name,Category, Property_Type, Project_Name,address, Area, phone_number, Owner_name, price,Cheque,Size, comments)
    )
    conn.commit()
    conn.close()
...
# ---------------- ADD ITEM ----------------
    ...
    #AREAS = [
        ...
        #"DLF Phase 2"
    #]
    Categories = ["Residential Land/Plot", "Kothi/Villa", "Builder floor", "Appartement/Flats", "Old/New Floors", "1RK/Studio Apartements", "Commercial"]
...
# ---------------- INVENTORY VIEW ----------------
    ...
    if st.session_state.role == "user":
        display_df["Owner Name"] = display_df["Owner Name"].apply(mask_name)
...
if st.session_state.role == "user":
    export_df["Owner Name"] = export_df["Owner Name"].apply(mask_name)
    export_df["Phone"] = export_df["Phone"].apply(mask_phone)

excel_buffer = io.BytesIO()
with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:

    export_df.to_excel(writer, index=False, sheet_name="All_Data")

    if not export_df.empty:
        for area in export_df["Area"].dropna().unique():
            area_df = export_df[export_df["Area"] == area]
            safe_sheet_name = area.replace("/", "-")[:31]
            area_df.to_excel(writer, index=False, sheet_name=safe_sheet_name)
